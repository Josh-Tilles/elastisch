...
;; TODO consider moving the opts into the document map with underscore prefixes
  ...
  ([^Connection conn index mapping-type id script params opts]
     (rest/post conn (rest/record-update-url conn
                ...
                {:body (merge {:script script :params params}
                              opts)})))
;; TODO rethink the above. Can probably combine `params` and `opts`, and maybe even `script`.
